...
class StockFlowFacade:
    ...
    def register_company(self, company_name, admin_name, admin_email, admin_password):
        ...
        self.repo.add_user(org_id, admin_name, admin_email, hashed, "admin")
    ...
    def create_new_user(self, admin_id, organization_id, name, email, password, role="user"):
        ...
        self.repo.add_user(organization_id, name, email, hashed, role)
    ...
    def update_product(self, requesting_user_id, organization_id, product_id, sku, name, alert_threshold):
        user_role = self.repo.get_user_role(requesting_user_id)
        if user_role != "admin":
            raise PermissionError("Seul un administrateur peut modifier un produit.")
        self.repo.update_product(organization_id, product_id, sku, name, alert_threshold)
