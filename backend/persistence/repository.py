...
class MySQLRepository:
    ...
    def update_product(self, organization_id, product_id, sku, name, alert_threshold):
        conn = self._get_connection()
        cursor = conn.cursor()
        try:
            ...
            fields, values = [], []
            if sku is not None:
                fields.append("sku = %s"); values.append(sku)
            if name is not None:
                fields.append("name = %s"); values.append(name)
            if alert_threshold is not None:
                fields.append("alert_threshold = %s"); values.append(alert_threshold)

            if not fields:
                return  # rien à mettre à jour

            values.extend([product_id, organization_id])
            cursor.execute(
                f"UPDATE products SET {', '.join(fields)} WHERE id = %s AND organization_id = %s",
                tuple(values)
            )
            if cursor.rowcount == 0:
                raise ValueError("Produit introuvable ou accès refusé.")
            conn.commit()
        except mysql.connector.Error as err:
            conn.rollback()
            if err.errno == 1062:
                raise ValueError("Ce SKU existe déjà dans votre organisation.")
            raise err
        finally:
            cursor.close()
            conn.close()
